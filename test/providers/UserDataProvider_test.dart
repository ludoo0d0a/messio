  ...
  group('UserDataProvider', () {
    ...
    test(
        'saveDetailsFromGoogleAuth returns a user with the details from FirebaseUser Object passed',
          ...
          expect(await documentReference.snapshots().isEmpty, true); //no data is saved for this user.
          User user = await userDataProvider.saveDetailsFromGoogleAuth(FirebaseUserMock());
          expect(await documentReference.snapshots().isEmpty, false); // data is saved. snapshots has one snapshot
    ...
    test(
        'saveDetailsFromGoogleAuth if there is existing image, do not write the image from firebase user',
          ...
          expect(user.name, 'John Doe');
          expect(user.photoUrl, 'http://www.google.com'); // for existing user the image is not overwritten.
        });
